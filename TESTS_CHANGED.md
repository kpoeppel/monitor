...
- Removed `tests/slurm_client/test_real_slurm_client.py` and the Fake SLURM client unit cases in `tests/clients/test_slurm_client.py` because SLURM client behavior now lives in the external `slurm_gen` library and monitor only wraps it.
