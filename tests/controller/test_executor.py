...
def test_executor_stop_job():
    ...
    reg = JobRegistration(name="test", command=["s"], log_path="l")
...
def test_executor_restart_job_with_adjustments():
    ...
    reg = JobRegistration(name="test", command=["s"], log_path="l", metadata={"a": 1})
    state = JobRuntimeState(job_id="123", registration=reg, submitted=True)
...
def test_executor_submit_array_job():
    ...
    reg = JobRegistration(name="array_test", command=["s"], log_path="l")
...
def test_executor_duplicate_job(monkeypatch):
    sub_mgr = MagicMock(spec=SubmissionManager)
    slurm = MagicMock(spec=MockSlurmClient)
    monitor = MagicMock(spec=BaseMonitor)
    slurm.submit.return_value = "dup_1"

    monkeypatch.setattr("monitor.executor.time.time", lambda: 123.0)
    executor = Executor(sub_mgr, slurm, monitor)

    reg = JobRegistration(name="test", command=["s"], log_path="l", extra_args=["--base"])
    state = JobRuntimeState(job_id="old123", registration=reg)

    job_id = executor.duplicate_job(
        state,
        adjustments={"extra_args_append": ["--new"]},
        name_suffix="-dup",
    )

    assert job_id == "dup_1"
    slurm.submit.assert_called_with("test-dup", ["s"], "l-dup_123000", ["--base", "--new"], True, None, None)
