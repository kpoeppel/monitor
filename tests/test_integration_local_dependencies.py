...
from monitor.watcher import SlurmLogMonitor, SlurmLogMonitorConfig
...
def test_local_file_dependency(tmp_path):
    ...
    monitor_config = SlurmLogMonitorConfig(poll_interval_seconds=1, log_path=str(tmp_path / "monitor.log"))
    ...
    reg1 = JobRegistration(
        name="producer",
        script_path=str(producer_script),
    ...
    reg2 = JobRegistration(
        name="consumer",
        script_path=str(consumer_script),
...
def test_cancel_condition_dependency(tmp_path):
    ...
    reg1 = JobRegistration(
        name="canceller",
        script_path=str(canceller_script),
    ...
    reg2 = JobRegistration(
        name="victim",
        script_path="echo 'I should not run'",
...
def test_complex_dependency_chain(tmp_path):
    ...
    reg1 = JobRegistration(
        name="starter",
        script_path=str(starter_script),
    ...
    reg2 = JobRegistration(
        name="canceler",
        script_path=str(canceler_script),
...
def test_idempotency_finish_condition(tmp_path):
    ...
    reg = JobRegistration(
        name="idempotent_job",
        script_path="echo 'Running...'", # Should not run
